...
```
⭐⭐⭐ 작업 시작 전 git pull origin dev 으로 최신화 ⭐⭐⭐
...
```bash
## 🚀 작업 시작 전 (필수!)
1. git switch dev : 병합 브랜치로 이동
2. git pull origin dev : 최신 코드 가져오기
3. git switch "브랜치명" : 본인 작업브랜치로 이동
4. git merge dev : dev 브랜치 최신코드 본인 브랜치로 병합

## 🛠 작업 완료 후 (푸쉬 방법)
# ※ 파이썬 라이브러리 추가 설치했을 때만 실행!
# pip freeze > requirements.txt
...
2. **PR은 기능 단위로 하나씩** 날려주세요. (하나의 PR에 여러 기능 혼재 금지)
3. 관리자가 코드 검수 및 로컬 테스트 후 병합을 승인합니다.

<br>

## 📝 참고사항 (Notes)
AI 모델 경로: AI 모델 파일은 대용량이므로 따로 공유합니다. 폴더에 수동으로 넣어주세요.
