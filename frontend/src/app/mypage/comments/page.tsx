...
export default function MyCommentsPage() {
  useEffect(() => {
    document.title = "Weather AI - 내 댓글";
  }, []);
          ...
          <div className={styles.statsRow}>
            {[
              ...
              <button
                ...
                onClick={() =>
                  setBoardFilter(boardFilter === s.key ? "전체" : s.key)
                }
              >
                ...
                {loading ? (
                  ...
                  filtered.map((c) => (
                    <tr
                      key={c.id}
                      className={styles.tr}
                      onClick={() => setDetailComment(c)}
                    >
                      <td className={styles.td}>
                        <span className={styles.idChip}>{c.id}</span>
                      </td>
                      <td className={styles.td}>
                        <span
                          className={`${styles.boardBadge} ${styles[TYPE_COLOR[c.post_board_type] ?? "suggest"]}`}
                        >
                          {TYPE_LABEL[c.post_board_type] ?? c.post_board_type}
                        </span>
                      </td>
                      <td className={styles.td}>
                        <span className={styles.postRef}>{c.post_title}</span>
                      </td>
                      <td className={styles.td}>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <div className={styles.modalMeta}>
                <span
                  className={`${styles.boardBadge} ${styles[TYPE_COLOR[detailComment.post_board_type] ?? "suggest"]}`}
                >
                  {TYPE_LABEL[detailComment.post_board_type] ??
                    detailComment.post_board_type}
                </span>
            ...
            <div className={styles.modalBody}>
              <div className={styles.modalPostRef}>
                ...
                <Link
                  href={`/board/${detailComment.board_id}`}
                  className={styles.modalPostTitle}
                >
              ...
              <div className={styles.modalCommentBox}>
                ...
                <p className={styles.modalCommentContent}>
