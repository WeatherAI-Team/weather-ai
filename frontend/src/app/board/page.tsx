...
export default function BoardPage() {
  ...
  const fetchPosts = useCallback(async () => {
    if (tab === 'bug') {
      setLoading(true)
      try {
        const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/board/posts?board_type=bug&per_page=200`)
        const data = await res.json()
        if (data.success) { setBugPosts(data.posts); setLoading(false); return }
      } catch {}
      setBugPosts([])
      setLoading(false)
      return
    ...
    if (tab === 'data') {
      setLoading(true)
      try {
        const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/board/posts?board_type=data&per_page=200`)
        const data = await res.json()
        if (data.success) { setPosts(data.posts); setLoading(false); return }
      } catch {}
      setPosts([])
      setLoading(false)
      return
  ...
  const handleBugStatusChange = (id: number, status: BugStatus) => {
    setBugPosts(prev => prev.map(p => p.id === id ? { ...p, status } : p))
    // TODO: API PATCH /api/board/posts/:id/status
  }
  ...
  const filteredBugPosts = bugStatusFilter === 'all'
    ? bugPosts
    : bugPosts.filter(p => p.status === bugStatusFilter)

  const filteredDataPosts = dataAuthorFilter === '전체'
    ? posts
            ...
            <>
              <div className={styles.filterRow}>
                {(['all', 'pending', 'in_progress', 'done'] as const).map(s => (
                  <button
                    key={s}
                    className={bugStatusFilter === s ? `${styles.filterBtn} ${styles.filterBtnActive}` : styles.filterBtn}
                    onClick={() => setBugStatusFilter(s)}
                  >
                    {s === 'all' ? '전체' : BUG_STATUS_LABELS[s]}
                    <span className={styles.filterCount}>
                      {s === 'all' ? bugPosts.length : bugPosts.filter(p => p.status === s).length}
                    </span>
                  </button>
                ))}
                <button
              ...
              <div className={styles.postList}>
                <div className={`${styles.listHeader} ${styles.listHeaderBug}`}>
                  <span>번호</span><span>제목</span><span>작성자</span><span>날짜</span><span>조회</span><span>상태</span>
                </div>
                {loading ? (
                  ...
                  <div key={post.id} className={`${styles.postRow} ${styles.postRowBug}`}>
                    <span className={styles.postNum}>{post.id}</span>
                    <Link href={`/board/${post.id}`} className={styles.postTitleLink}>{post.title}</Link>
                    ...
                    <span className={styles.statusCell}>
                      {isAdmin(userRole) ? (
                        <select
                          ...
                          value={post.status}
                          onChange={e => handleBugStatusChange(post.id, e.target.value as BugStatus)}
                        >
            ...
            <>
              <div className={styles.filterRow}>
                {DATA_AUTHORS.map(author => (
                  <button
                    key={author}
                    className={dataAuthorFilter === author ? `${styles.filterBtn} ${styles.filterBtnActive}` : styles.filterBtn}
                    onClick={() => setDataAuthorFilter(author)}
                  >
                    {author}
                    {author !== '전체' && (
                      <span className={styles.filterCount}>
                        {posts.filter(p => p.author_nickname === author).length}
                      </span>
                    )}
                  </button>
                ))}
              ...
              <div className={styles.postList}>
                <div className={`${styles.listHeader} ${styles.listHeaderData}`}>
                  <span>번호</span><span>제목</span><span>작성자</span><span>날짜</span><span>첨부</span><span>조회</span>
                </div>
                {loading ? (
                  <div className={styles.empty}>불러오는 중...</div>
                  ...
                  <Link key={post.id} href={`/board/${post.id}`} className={`${styles.postRow} ${styles.postRowData}`}>
                    <span className={styles.postNum}>{post.id}</span>
                    <span className={styles.postTitle}>{post.title}</span>
                    <span className={styles.author}>{post.author_nickname}</span>
                    <span className={styles.date}>{post.created_at}</span>
                    <span className={styles.commentCol}>{post.attachment_count ?? 0}</span>
                    <span className={styles.views}>👁 {post.views}</span>
                  </Link>
                ))}
              </div>
            ...
            <>
              ...
              <div className={styles.postList}>
                <div className={styles.listHeader}>
                  <span>번호</span><span>카테고리</span><span>제목</span><span>작성자</span><span>날짜</span><span>댓글</span><span>조회</span>
                </div>
